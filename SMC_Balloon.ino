...
#define actuator_close 135
#define actuator_open 45
...
//    int signal_strength;
  ...
  //IMU
   ...
   //Mode first-run variables
     int space_mode_first_run = 0;
...
{ 
  ...
  for(int initPin = 4; initPin < 10 ; initPin++){
  ...
  digitalWrite(system_ready_indicator,HIGH);
  ...
  Wire.begin(); //Initialize TWI 
  Serial.println("Serials ok");  
  //openLogInit(); //Initialize OpenLog
  ...
  actuator.attach(5);// Actuator on pin 5
  actuator.write(actuator_close); // Make sure actuator is in closed position
  delay(500); // Wait for servo to reach closed position
  actuator.detach(); // Release servo object while not in use
  pitch.attach(12);
  yaw1.attach(14);
  yaw2.attach(15);
  roll.attach(20);
...
void space()
{
  /* IN-SPACE ROUTINE
        ...
        Check for freefall
  */
  if(space_mode_first_run == 0){
    space_mode_first_run = 1;
...
void getPressure(){
  ...
  Serial.print(pres);
  Serial.println(" to PRESSURE.txt");
  //TEMP
}
...
//  Serial1.print("Humidity: "); Serial1.print(humidity); Serial1.println("%rh");
  ...
  Serial.print("Appending ");
  Serial.print(hum);
  Serial.println(" to HUMIDITY.txt");
  return humidity;
}
...
int getAmbientLight(){
  ...
  //DEFINE AMBIENT LIGHT PIN
   ...
   Serial.print("Appending ");
   Serial.print(amb);
   Serial.println(" to AMBILGHT.txt");
